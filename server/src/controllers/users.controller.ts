...
export const registerUser = catchAsyncError(async (req: Request, res: Response) => {
  ...
  res.status(HttpStatusCode.CREATED).json(new ApiResponse("User created successfully"));
});
...
/**Change password */
export const changePassword = catchAsyncError(async (req: AuthRequest, res: Response) => {
  const { oldPassword, newPassword } = req.body;
  const { id } = req.user;
  await UserService.changePassword({ id, oldPassword, newPassword });
  res.status(HttpStatusCode.OK).json(new ApiResponse("Password successfully changed."));
});

/**Get user by email */
export const getUserById = catchAsyncError(async (req: AuthRequest, res: Response) => {
  const { id } = req.user;
...
export const updateUser = catchAsyncError(async (req: AuthRequest, res: Response) => {
  ...
  res.status(HttpStatusCode.OK).json(new ApiResponse("User updated successfully"));
});

/**update user profile */
export const updateUserProfile = catchAsyncError(async (req: AuthRequest, res: Response) => {
  const { id } = req.user;
  ...
  res.status(HttpStatusCode.OK).json(new ApiResponse("User profile updated successfully"));
});
