...
export const getLikeStatus = catchAsyncError(async (req: AuthRequest, res: Response) => {
...
export const updateLikeCount = catchAsyncError(async (req: AuthRequest, res: Response) => {
  const { id: userId } = req.user;
  const { videoPublicId } = req.params;
